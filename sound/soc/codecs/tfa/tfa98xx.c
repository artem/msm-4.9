...
#ifdef CONFIG_SH_AUDIO_DRIVER /*A-004*/
#ifdef CONFIG_SND_SOC_TREBLE_ENABLE
static BLOCKING_NOTIFIER_HEAD(tfa98xx_shub_api_notifier_list);

int register_tfa98xx_shub_api_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&tfa98xx_shub_api_notifier_list, nb);
}
EXPORT_SYMBOL(register_tfa98xx_shub_api_notifier);

int unregister_tfa98xx_shub_api_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&tfa98xx_shub_api_notifier_list, nb);
}
...
unsigned short tfa98xx_get_hw_revision(void)
{
	unsigned short hw_rev = 0;
#ifdef CONFIG_SHARP_BOOT
	hw_rev = sh_boot_get_hw_revision();
#endif
	if (hw_rev == 0xFF) {
		pr_err("%s: sh_boot_get_hw_revision fail!, force return 0x00", __FUNCTION__);
		hw_rev = 0x00;
	}
	return hw_rev;
}
...
int tfa98xx_set_mtpex(int val) {
	...
	if (val != 0) {
		pr_err("%s(): Can only clear MTPEX (0 value expected)\n", __FUNCTION__);
		return -EINVAL;
	}

	mtpex->deferrable = true;
	mutex_lock(&tfa98xx->dsp_lock);
	status = tfa98xx_open(tfa98xx->handle);
	if (status) {
		mutex_unlock(&tfa98xx->dsp_lock);
...
#define NXP_UNSET_CALIBRATION 0
#define NXP_ENABLE_CALIBRATION 1
#define NXP_RESET_CALIBRATION 2
int tfa98xx_enable_cal_bit(int func)
{
	int handle = 0, err = 0;
	...
	if (tfa98xx_devices[handle]) {
		switch (func) {
		case NXP_UNSET_CALIBRATION:
			handles_local[tfa98xx_devices[handle]->handle].forceUnset = true;
			pr_debug("Enable force unset MTP/OTC after DSP is up\n");
			break;
		case NXP_ENABLE_CALIBRATION:
			err = tfa98xx_set_otc(1);
			if (err) {
				pr_err("%s(): failed to set otc\n", __FUNCTION__);
				return err;
			}

			handles_local[tfa98xx_devices[handle]->handle].forceCal = true;
			pr_debug("Set 1 > otc in cache, and force calibration\n");
			break;
		case NXP_RESET_CALIBRATION:
			err = tfa98xx_set_otc(0);
			if (err) {
				pr_err("%s(): failed to reset otc\n", __FUNCTION__);
				return err;
			}

			err = tfa98xx_set_mtpex(0);
			if (err) {
				pr_err("%s(): failed to reset mtpex\n", __FUNCTION__);
				return err;
			}
			break;
		default:
			err = -EINVAL;
			pr_err("%s(): Unknown function (%d)\n", __FUNCTION__, func);
		}

		return err;
	} else {
		pr_err("TFA98XX does not init\n");
		return -EINVAL;
	}
}
#ifndef CONFIG_SND_SOC_TREBLE_ENABLE
EXPORT_SYMBOL_GPL(tfa98xx_enable_cal_bit);
#endif /* CONFIG_SND_SOC_TREBLE_ENABLE */

int tfa98xx_check_mtpex(int *val)
{
	int handle = 0;
	enum Tfa98xx_Error err, status;
	...
	if (tfa98xx_devices[handle]) {
		...
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			pr_debug("Unable to check DSP because of no clock, force return 2\n");
			*val = 2;
			return 0;
		}
...
#ifndef CONFIG_SND_SOC_TREBLE_ENABLE
EXPORT_SYMBOL_GPL(tfa98xx_check_mtpex);
#endif /* CONFIG_SND_SOC_TREBLE_ENABLE */

int tfa98xx_check_impedance(int *val)
{
	int handle = 0;

	if (tfa98xx_devices[handle]) {

		*val = handles_local[handle].mohm[0];

		pr_debug("Impedance: %d mOhms \n", *val);
		return 0;
	} else {
		pr_err("TFA98XX does not init\n");
		return -EINVAL;
...
#ifndef CONFIG_SND_SOC_TREBLE_ENABLE
EXPORT_SYMBOL_GPL(tfa98xx_check_impedance);
#endif /* CONFIG_SND_SOC_TREBLE_ENABLE */
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
#ifdef SH_DUMP_ENG
static int enable_regs_dump = 0;
module_param(enable_regs_dump, int, S_IRUGO | S_IWUSR);
unsigned int allRegsAddr[25] = {
	0x00, 0x01, 0x02, 0x03,
	0x04, 0x05, 0x06, 0x07,
	0x08, 0x09, 0x0a, 0x0b,
	0x0f, 0x10, 0x11, 0x12,
...
static void tfa98xx_regs_dump(struct tfa98xx *tfa98xx) {
	unsigned int allRegsValue[25] = {0};
	unsigned char debugInfo[512];
	unsigned char buf[20];
	int i;

	memset(buf, 0, ARRAY_SIZE(buf));
	memset(debugInfo, 0, ARRAY_SIZE(debugInfo));
	for(i = 0; i < ARRAY_SIZE(allRegsAddr); i++) {
		regmap_read(tfa98xx->regmap, allRegsAddr[i], &allRegsValue[i]);
		sprintf(buf, "0x%02x = 0x%04x, ", allRegsAddr[i], allRegsValue[i]);
		strcat(debugInfo, buf);
		memset(buf, 0, ARRAY_SIZE(buf));
		if ((i+1)%5 == 0 || i == (ARRAY_SIZE(allRegsAddr) - 1)) {
			pr_debug("%s\n", debugInfo);
			memset(debugInfo, 0, ARRAY_SIZE(debugInfo));
		}
	}
...
#ifdef SH_DUMP_ENG
		if (enable_regs_dump) {
			pr_debug("Dump at monitor\n");
			tfa98xx_regs_dump(tfa98xx);
		}
#endif	/* SH_DUMP_ENG */
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	mutex_unlock(&tfa98xx->dsp_lock);
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
	if (handles_local[0].forceUnset) {
		int err = 0;

		err = tfa98xx_set_otc(0);
		if (err) {
			pr_err("%s(): failed to unset otc\n", __FUNCTION__);
		} else {
			err = tfa98xx_set_mtpex(0);
			if (err) {
				pr_err("%s(): failed to unset mtpex\n", __FUNCTION__);
			} else {
				handles_local[0].forceUnset = false;
				pr_debug("Force unset MTPEX/OTC successed\n");
			}
		}
	}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
	return 0;
#else	/* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
//		shub_api_restart_pedometer_func(SHUB_STOP_PED_TYPE_SPE); /* LKM Sensorhub del */
		blocking_notifier_call_chain(&tfa98xx_shub_api_notifier_list, 0, NULL);
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK)
			...
			tfa98xx->cstream = 1;
#ifdef CONFIG_SH_AUDIO_DRIVER /* B-019 */
//		shub_api_stop_pedometer_func(SHUB_STOP_PED_TYPE_SPE); /* LKM Sensorhub del */
		blocking_notifier_call_chain(&tfa98xx_shub_api_notifier_list, 1, NULL);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* B-019 */
		/* Start DSP */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
static int tfa98xx_panic_handler(struct notifier_block *this,
	unsigned long event, void *ptr)
{
	struct tfa98xx *tfa98xx = tfa98xx_devices[0];
	tfa98xx_ext_reset(tfa98xx);
	pr_debug("reset in panic\n");
	return NOTIFY_DONE;
}

static struct notifier_block panic_nb = {
	.notifier_call = tfa98xx_panic_handler,
	.priority = 1,
};
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
#ifndef CONFIG_SH_AUDIO_DRIVER /* A-004 */
		.capture = {
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
struct snd_soc_codec *tfa98xx_codec = NULL;
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
	tfa98xx_codec = codec;
	pr_err("tfa98xx early probe successed!\n");
	return 0;
}

int tfa98xx_later_probe(void)
{
	struct snd_soc_codec *codec = tfa98xx_codec;
	unsigned short hw_rev = 0;
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	int ret;
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
	hw_rev = tfa98xx_get_hw_revision();
	pr_debug("HW revision is 0x%x", hw_rev);
	if (hw_rev >= HW_REVISION_PP2) {
		fw_name = "tfa98xx_rev2.cnt";
	}
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
static bool tfa98xx_available = true;
bool tfa98xx_detected(void)
{
	return tfa98xx_available;
}

#ifdef CONFIG_SND_SOC_TREBLE_ENABLE
static int tfa98xx_notifier_handler_func(struct notifier_block *nb,
                                unsigned long function, void *val)
{
	int value = 0;

	switch (function) {
	case TFA98XX_NOTIFIER_DETECTED:
		return tfa98xx_detected();
	case TFA98XX_NOTIFIER_LATER_PROBE:
		return tfa98xx_later_probe();
	case TFA98XX_NOTIFIER_ENABLE_CAL_BIT:
		if (val != NULL) {
			value = *(int *)val;
			return tfa98xx_enable_cal_bit(value);
		}
		else
			return -EINVAL;
	case TFA98XX_NOTIFIER_CHECK_MTPEX:
		if (val != NULL)
			return tfa98xx_check_mtpex(val);
		else
			return -EINVAL;
	case TFA98XX_NOTIFIER_CHECK_IMPEDANCE:
		if (val != NULL)
			return tfa98xx_check_impedance(val);
		else
			return -EINVAL;
	default:
...
static struct notifier_block tfa98xx_notifier_handler = {
		.notifier_call = tfa98xx_notifier_handler_func,
};
#endif /* CONFIG_SND_SOC_TREBLE_ENABLE */
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->codec == NULL){
		pr_err("The tfa98xx->codec is NULL\n");
		return;
	}
	val = snd_soc_read(tfa98xx->codec, TFA98XX_STATUSREG);
	...
	if (no_start == 0) {
		...
		if (ret < 0) {
			dev_err(&i2c->dev, "Failed to read Revision register: %d\n",
				ret);
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
			dev_err(&i2c->dev, "ERROR!! Can't detect tfa98xx\n");
			tfa98xx_available = false;
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
			return -EIO;
		}
		switch (reg & 0xff) {
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
#ifdef CONFIG_SND_SOC_TREBLE_ENABLE
	ret = register_tfa98xx_snd_notifier(&tfa98xx_notifier_handler);
	if (ret)
		dev_err(&i2c->dev, "error registering notifier\n");
	ret = register_tfa98xx_sdm845_notifier(&tfa98xx_notifier_handler);
	if (ret)
		dev_err(&i2c->dev, "error registering notifier\n");
#endif /* CONFIG_SND_SOC_TREBLE_ENABLE */
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
#ifdef CONFIG_SND_SOC_TREBLE_ENABLE
	unregister_tfa98xx_snd_notifier(&tfa98xx_notifier_handler);
	unregister_tfa98xx_sdm845_notifier(&tfa98xx_notifier_handler);
#endif /* CONFIG_SND_SOC_TREBLE_ENABLE */
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
