...
#define YAS539_SLAVE_ADDRESS            (0x2Eu)
#define YAS539_BUSY_RETRY_MAX           (10)
#define YAS_RCOIL_RETRY_MAX             (1)
#define YAS_RCOIL_RETRY_TIME_US         (1000000)
...
#define YAS_CAL_REG_NUM                 (13)
#define YAS_PCB_MEASURE_DATA_REG_NUM    (8)
#define YAS_PCB_MEASURE_COMMAND_START   (0x81)
#define YAS_PCB_MEASURE_COMMAND_TC_SEN  (0x40)

#define YAS539_CAL_EOF                  (32768)
#define YAS539_CAL_OF_UF                (16384)
...
static int shub_diag_mag_initialize(void)
{
    ...
    /* SRSTR Soft reset */
    setData = 0x80;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_SRSTR, setData);
    if(ret != 0) {
        return -1;
    }
    return 0;
...
static int shub_diag_mag_set_coil(void)
{
    ...
    /* RCDRVR */
    setData = 0x01;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_RCDRVR, setData);
    if(ret != 0) {
        return -1;
    }
    /* RCOILR rcoil */
    setData = 0x0a;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_RCOILR, setData);
    if(ret != 0) {
        return -1;
    }
    usleep(YAS539_MAG_RCOIL_TIME0);
#else
    /* CONFR Config set */
#ifdef SHUB_SW_MAG_INT /* SHMDS_HUB_0103_20 */
    setData = 0x09;
#else
    setData = 0x08;
#endif
...
static int shub_diag_mag_initialize_2(void)
{
    ...
    /* ADCCALR A/D rev */
    setData = 0x03;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_ADCCALR, setData);
    if(ret != 0) {
        return -1;
    }
    setData = 0xE0;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_ADCCALR + 1, setData);
    if(ret != 0) {
        return -1;
    }
    /* TRMR Temperature rev */
    setData = 0xFF;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_TRMR, setData);
    if(ret != 0) {
        return -1;
    }
    ...
    /* RCAUTOR */
    setData = 0x8a;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_RCAUTOR, setData);
    if(ret != 0) {
        return -1;
    }
    /* RCCURR */
    setData = 0x03;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_RCCURR, setData);
    if(ret != 0) {
        return -1;
    }
    /* INTRVLR Interval */
    setData = 0x00;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_INTRVLR, setData);
    if(ret != 0) {
        return -1;
    }
    /* AVRR Average filter */
    setData = 0x2D;
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_AVRR, setData);
    if(ret != 0) {
        return -1;
    }
#else
...
//    int16_t a[3];
...
#ifdef USE_YAS539
    const int16_t h[] = {16000, 16256, 16512, 16768};
    ...
    ysaCalData.fxy1y2[0] = (pu08Data[12]>>4)&0x03;
    ysaCalData.fxy1y2[1] = (pu08Data[12]>>2)&0x03;
    ysaCalData.fxy1y2[2] = pu08Data[12]&0x03;

    for (i = 0; i < 3; i++) {
        eof[i] = YAS539_CAL_EOF - h[ysaCalData.fxy1y2[i]] - SHUB_ABS(ysaCalData.cxy1y2[i]) * 225 / 8;
        euf[i] = h[ysaCalData.fxy1y2[i]] - SHUB_ABS(ysaCalData.cxy1y2[i]) * 225 / 8;
    }

    of[0] = YAS539_CAL_OF_UF + ysaCalData.k * (ysaCalData.a1 * eof[0] - SHUB_ABS(ysaCalData.a2) * eof[1] - SHUB_ABS(ysaCalData.a3) * eof[2] ) / 8192;
    uf[0] = YAS539_CAL_OF_UF - ysaCalData.k * (ysaCalData.a1 * euf[0] - SHUB_ABS(ysaCalData.a2) * euf[1] - SHUB_ABS(ysaCalData.a3) * euf[2] ) / 8192;
    of[1] = YAS539_CAL_OF_UF + ysaCalData.k * (-SHUB_ABS(ysaCalData.a4) * eof[0] + ysaCalData.a5 * eof[1] - SHUB_ABS(ysaCalData.a6) * eof[2] ) / 8192;
    uf[1] = YAS539_CAL_OF_UF - ysaCalData.k * (-SHUB_ABS(ysaCalData.a4) * euf[0] + ysaCalData.a5 * euf[1] - SHUB_ABS(ysaCalData.a6) * euf[2] ) / 8192;
    of[2] = YAS539_CAL_OF_UF + ysaCalData.k * (-SHUB_ABS(ysaCalData.a7) * eof[0] - SHUB_ABS(ysaCalData.a8) * eof[1] + ysaCalData.a9 * eof[2] ) / 8192;
    uf[2] = YAS539_CAL_OF_UF - ysaCalData.k * (-SHUB_ABS(ysaCalData.a7) * euf[0] - SHUB_ABS(ysaCalData.a8) * euf[1] + ysaCalData.a9 * euf[2] ) / 8192;

    for (i = 0; i < 3; i++) {
        yas_overflow[i] = of[i] > YAS539_DATA_OVERFLOW ? YAS539_DATA_OVERFLOW : of[i];
        yas_underflow[i] = uf[i] < YAS539_DATA_UNDERFLOW ? YAS539_DATA_UNDERFLOW : uf[i];
    }
#else
    ysaCalData.cxy1y2[0] = ((pu08Data[0] << 1) | (pu08Data[1] >> 7)) - 256;
    ysaCalData.cxy1y2[1] = (((pu08Data[1] << 2) & 0x1fc) | (pu08Data[2] >> 6)) - 256;
...
static int shub_diag_mag_read_cal(void)
{
    ...
    if(read_cal_init_flg == 0) {
        ...
        {
#ifdef USE_YAS539
            ret = shub_diag_magSensorReadSendCmd(YAS539_REG_CALR + i, resData);
#else
...
static int shub_diag_mag_measure(uint8_t* mesData, uint8_t cmd, int portCheck)
{
...
#ifdef USE_YAS539
    ret = shub_diag_magSensorWriteSendCmd(YAS539_REG_CMDR, cmd);
#else
    ...
    do {
#ifdef USE_YAS539
        if(busy_cnt >= YAS539_BUSY_RETRY_MAX) {
        ...
        /* F_MB reg read */
        memset(resData, 0, sizeof(resData));
        ret = shub_diag_magSensorReadSendCmd(YAS539_REG_DATAR + 2, resData);
        if(ret != 0) {
...
#ifdef USE_YAS539
    ret = shub_diag_sensorMultiReadSendCmd(YAS539_REG_DATAR, mesData, YAS539_SLAVE_ADDRESS, YAS_PCB_MEASURE_DATA_REG_NUM);
#else
...
static int shub_diag_mag_data_measure(int32_t* revData)
{
    ...
    for(mes_cnt = 0; mes_cnt <= YAS_RCOIL_RETRY_MAX; mes_cnt++) {
        if(rcoil_setting) {
            DBG_DIAG_IO("---Mag measurement retry[%d](cause=%d)\n", mes_cnt, rcoil_setting);
            /* wait */
            usleep(YAS_RCOIL_RETRY_TIME_US);
            /* coil data setting */
            ret = shub_diag_mag_set_coil();
            if(ret != 0) {
...
#ifdef USE_YAS539
        rcoil = 0;
        temperature = (uint16_t)(((mesData[1] & 0x3f) << 8) | mesData[1+1]);
        xy1y2[0] = (uint16_t)(((mesData[2+1] & 0x7f) << 8) | mesData[3+1]);
        xy1y2[1] = (uint16_t)(((mesData[4+1] & 0x7f) << 8) | mesData[5+1]);
        xy1y2[2] = (uint16_t)(((mesData[6+1] & 0x7f) << 8) | mesData[7+1]);
#else
...
#ifdef USE_YAS539
        for (i = 0; i < 3; i++) {
            oc[i] = (xy1y2[i] - 16384) * 100000 / (100000 - 11 * (temperature - 8170));
        }

        /* xy1y2 to xyz(uT) change */
        revData[0] = (oc[0] * 150) / 1000;
        revData[1] = ((oc[1] - oc[2]) * 1732 / 10 / 2) /1000;
        revData[2] = ((-oc[1] - oc[2]) * 150) / 1000;

        DBG_DIAG_IO("Mag mes(rawchg),t=%d,x=%d,y1=%d,y2=%d, mes(uT),x=%d,y=%d,z=%d\n", temperature, xy1y2[0], xy1y2[1], xy1y2[2], revData[0], revData[1], revData[2]);
        return 0;
#else
        if (ysaCalData.ver == 1) {
            /* coordinate change  */
...
/* SHMDS_HUB_0341_01 mod S */
            /* xy1y2 to xyz(uT) change */
            revData[0] = ((xy1y2[0] -8192) * 300) / 1000;
            revData[1] = ((xy1y2[1] - xy1y2[2]) * 1732 / 10) / 1000;
            revData[2] = ((-xy1y2[1] - xy1y2[2] + 16384) * 300) / 1000;
/* SHMDS_HUB_0341_01 mod E */
...
static int shub_diag_mag_self_test_check(int32_t *data)
{
    ...
    /* mag measure plus */
    cmd = YAS_PCB_MEASURE_COMMAND_START | YAS_PCB_MEASURE_COMMAND_TC_SEN;
    ret = shub_diag_mag_measure(mesData, cmd, 1);
    if(ret != 0) {
        return -1;
    }
    data[0] = (uint16_t)(((mesData[2+1] & 0x7f) << 8) | mesData[3+1]);
    data[1] = (uint16_t)(((mesData[4+1] & 0x7f) << 8) | mesData[5+1]);
    data[2] = (uint16_t)(((mesData[6+1] & 0x7f) << 8) | mesData[7+1]);
    
    if (((16544 <= data[0]) && (data[0] <= 17024)) &&
        ((16517 <= data[1]) && (data[1] <= 17184)) &&
        ((15584 <= data[2]) && (data[2] <= 16251))) {
        /* OK */
        printk("[shub]%s self test OK. x=%d y1=%d y2=%d\n", __func__, data[0], data[1], data[2]);
        ret = 0;
    } else {
        /* OK */
        printk("[shub]%s self test NG. x=%d y1=%d y2=%d\n", __func__, data[0], data[1], data[2]);
        ret = -2;
    }
    return ret;
#else
...
/* SHMDS_HUB_0351_01 add S */
                    if(res.rtn != 0){
                        printk("error : shub_ioctl(cmd = SHUB_DIAG_MES_GYRO) shub_diag_gyro_data_measure\n");
                    }

                    ret = shub_diag_gyro_mode_change(0);
                    if(ret != 0){
                        printk("error : shub_ioctl(cmd = SHUB_DIAG_MES_GYRO) shub_diag_gyro_mode_change power down\n");
                        return -EFAULT;
                    }
/* SHMDS_HUB_0351_01 add E */
                }
...
#ifdef USE_YAS539
                DBG_DIAG_IO("ioctl(cmd = Mag_Self_Test) : ret=%d, val=%d, %d, %d\n", res.rtn, res.magData[0], res.magData[1], res.magData[2]);
#else
...
int shub_diag_init(void)          /* SHMDS_HUB_3701_01 mod */
{
    int ret;
...
void shub_diag_exit(void)         /* SHMDS_HUB_3701_01 mod */
{
    platform_driver_unregister(&shub_diag_driver);
}

// late_initcall(shub_diag_init); /* SHMDS_HUB_3701_01 del */
// module_exit(shub_diag_exit);   /* SHMDS_HUB_3701_01 del */
