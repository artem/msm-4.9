/* drivers/input/touchscreen/shtps/sy3000/shtps_i2c.c
 *
...
#define SHTPS_I2C_BLOCKREAD_BUFSIZE 100
...
static int shtps_i2c_error_detect_flg = 0;
...
/* -----------------------------------------------------------------------------------
 */
int shtps_get_i2c_error_detect_flg(void)
{
	return shtps_i2c_error_detect_flg;
}

void shtps_clr_i2c_error_detect_flg(void)
{
	shtps_i2c_error_detect_flg = 0;
	return;
}
...
static int shtps_i2c_write_main(void *client, u16 addr, u8 *writebuf, u32 size)
{
	...
	buf[0] = 0xff;
	buf[1] = (addr >> 8) & 0xff;
	ret = shtps_i2c_transfer((struct i2c_client *)client, buf, 2, NULL, 0);
	if(ret < 0){
		SHTPS_LOG_ERR_PRINT("i2c write error. page select(%d)\n", ((addr >> 8) & 0xff));
	}
	else{
		buf[0] = addr & 0xff;
...
static int shtps_i2c_read_main(void *client, u16 addr, u8 *readbuf, u32 size)
{
	...
	if(addr != 0x7FFF){
		...
		writebuf[0] = 0xff;
		writebuf[1] = (addr >> 8) & 0xff;
		ret = shtps_i2c_transfer((struct i2c_client *)client, writebuf, 2, NULL, 0);
		if(ret < 0){
			SHTPS_LOG_ERR_PRINT("i2c read error. page select(%d)\n", ((addr >> 8) & 0xff));
		}
		else{
			writebuf[0] = addr & 0xff;
...
static int shtps_i2c_remove(struct i2c_client *client)
{
	...
	struct shtps_rmi_spi *ts_p = i2c_get_clientdata(client);
	int ret = shtps_rmi_core_remove(ts_p, &client->dev);
...
/* -----------------------------------------------------------------------------------
*/
static const struct dev_pm_ops shtps_rmi_pm_ops = {
	.suspend	= shtps_i2c_suspend,
	.resume		= shtps_i2c_resume,
};
static const struct i2c_device_id shtps_rmi_id[] = {
	{"shtps_rmi", 0},
	{},
};

MODULE_DEVICE_TABLE(i2c, shtps_rmi_id);

#ifdef CONFIG_OF // Open firmware must be defined for dts useage
static struct of_device_id shtps_rmi_table[] = {
	{ . compatible = "sharp,shtps_rmi-i2c" ,}, // Compatible node must match dts
	{ },
};
#endif
...
/* -----------------------------------------------------------------------------------
*/
#endif /* defined( SHTPS_DEF_FWCTL_PROTOCOL_I2C ) */
