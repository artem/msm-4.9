...
static void tfa_set_query_info (int dev_idx) {
	...
	handles_local[dev_idx].forceCal = false;
	handles_local[dev_idx].forceUnset = false;
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* A-004 */
	if (handles_local[handle].forceCal) {
		force = 1;
		handles_local[handle].forceCal = false;
		pr_debug("%s(): Force calibration\n", __FUNCTION__);
	}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* A-004 */
