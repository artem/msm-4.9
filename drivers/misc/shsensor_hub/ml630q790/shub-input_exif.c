...
#ifdef SHUB_SW_NOTIFIER
#include <linux/input/shtps_dev.h>          /* SHMDS_HUB_3702_01 add */
#include <linux/leds-qpnp-haptics.h>        /* SHMDS_HUB_3702_01 add */
#include <sharp/shaudio_tfa98xx.h>          /* SHMDS_HUB_3702_01 add */
#endif
...
void shub_sensor_rep_input_exif(struct seq_file *s)
{
    ...
    seq_printf(s, "[exif      ]shub_not_notify_speaker_flg=%d\n",shub_not_notify_speaker_flg);    /* SHMDS_HUB_0208_04 add */
}
// SHMDS_HUB_0701_05 add E
...
static struct notifier_block shub_vibration_pedometer_handler = {
    .notifier_call = shub_vibration_pedometer_handler_func,
};
static int shub_audio_pedometer_handler_func(struct notifier_block *nb,
                                                  unsigned long on, void *unused)
{
    if(on == 1){
...
static struct notifier_block shub_audio_pedometer_handler = {
    .notifier_call = shub_audio_pedometer_handler_func,
};
#endif
/* SHMDS_HUB_3702_01 add E */
...
/* SHMDS_HUB_3702_01 add S */
    ...
    if (ret) {
        printk("%s: vibration notifier register failed\n", __func__);
        goto out_driver;
    }
    ret = register_tfa98xx_shub_api_notifier(&shub_audio_pedometer_handler);
    if (ret) {
        printk("%s: audio notifier register failed\n", __func__);
        goto out_driver;
    }
#endif
/* SHMDS_HUB_3702_01 add E */
...
/* SHMDS_HUB_3702_01 add S */
    ...
    unregister_tfa98xx_shub_api_notifier(&shub_audio_pedometer_handler);
#endif
/* SHMDS_HUB_3702_01 add E */
...
int shub_exif_init(void)          /* SHMDS_HUB_3701_01 mod */
{
    int ret;
...
void shub_exif_exit(void)         /* SHMDS_HUB_3701_01 mod */
{
    platform_driver_unregister(&shub_exif_driver);
}

// late_initcall(shub_exif_init); /* SHMDS_HUB_3701_01 del */
// module_exit(shub_exif_exit);   /* SHMDS_HUB_3701_01 del */
