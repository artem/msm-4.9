/* drivers/input/touchscreen/shtps/sy3000/shtps_rmi_sub.c
 *
...
static char* shtps_func_get_event_name(int event);
...
int shtps_proximity_check(struct shtps_rmi_spi *ts)
{
	...
	if(data == SHTPS_PROXIMITY_NEAR){
		return SHTPS_PROXIMITY_NEAR;
	}

	return SHTPS_PROXIMITY_FAR;
}
#else
static void shtps_proximity_get_index(struct shtps_rmi_spi *ts)
{
	...
	for(index = 0; index < 30; index++){
		...
		sprintf(devname,"/sys/devices/virtual/input/input%d/name",index);
		file = filp_open( devname, O_RDONLY, 0);
		if(IS_ERR_OR_NULL(file) == 0){
			old_fs = get_fs();
			set_fs(get_ds());
			...
			if( (file->f_op->read(file, name, sizeof(name), &file->f_pos)) < 0 ){
				...
				name[strlen(SHTPS_PROXIMITY_DEVICE_NAME_STR)] = '\0';
			}
			set_fs(old_fs);
			filp_close(file, NULL);
			if( strcmp(name, SHTPS_PROXIMITY_DEVICE_NAME_STR) == 0 ){
				ts->proximity_input_index = index;
				SHTPS_LOG_DBG_PRINT("[proximity] input index found <%d>\n", ts->proximity_input_index);
				break;
			}
		}
	}
...
static int shtps_proximity_get_info(struct shtps_rmi_spi *ts, const char *control_req_p)
{
	int ret = -1;
	int read_size;
	char value[4];
	...
	if(ts->proximity_input_index < 0){
		shtps_proximity_get_index(ts);
	}
	if(ts->proximity_input_index >= 0){
		sprintf(devname,"/sys/devices/virtual/input/input%d/%s",ts->proximity_input_index, control_req_p);
		file = filp_open(devname, O_RDONLY, 0);
		if(IS_ERR_OR_NULL(file)){
			SHTPS_LOG_DBG_PRINT("[proximity] <req:%s> Cannot open file. err\n", control_req_p);
			ret = -1;
		}else{
			old_fs = get_fs();
			set_fs(get_ds());
			read_size = file->f_op->read(file, value, sizeof(value), &file->f_pos);
			set_fs(old_fs);
			filp_close(file, NULL);

			if(read_size > 0){
				if( strcmp( value, SHTPS_PROXIMITY_ERROR_STR ) == 0 ){
					SHTPS_LOG_DBG_PRINT("[proximity] <req:%s> error\n", control_req_p);
					ret = -1;
				}else{
					ret = simple_strtoul( value, NULL, 10 );
				}
			}else{
				SHTPS_LOG_DBG_PRINT("[proximity] <req:%s> read failed\n", control_req_p);
				ret = -1;
			}
		}
	}else{
		SHTPS_LOG_DBG_PRINT("[proximity] <req:%s> input index not find\n", control_req_p);
	}
...
int shtps_proximity_state_check(struct shtps_rmi_spi *ts)
{
	...
	data = shtps_proximity_get_info(ts, "ps_distance");
	SHTPS_LOG_DBG_PRINT("[proximity] check state end(data:%d)\n",data);
...
int shtps_proximity_check(struct shtps_rmi_spi *ts)
{
	...
	SHTPS_LOG_DBG_PRINT("[proximity] check start\n");
	data = shtps_proximity_get_info(ts, "ps_dataread");
	SHTPS_LOG_DBG_PRINT("[proximity] check end(data:%d)\n",data);

	if(data < 0){
		data = SHTPS_PROXIMITY_FAR;
	}
...
void shtps_lpwg_proximity_check_delayed_work_function(struct work_struct *work)
{
	...
	SHTPS_LOG_FUNC_CALL();
	shtps_func_request_async(ts, SHTPS_FUNC_REQ_EVENT_PROXIMITY_CHECK);
}
...
static DEFINE_PER_CPU(unsigned int, baselimit_freq) = 0;
...
static void shtps_perflock_cluster0_enable(struct shtps_rmi_spi *ts)
{
	...
	if(req == 1){
		if(ts->fw_report_rate_cur == SHTPS_CTRL_FW_REPORT_RATE_PARAM_HIGH){
			ts->cpu_clock_ctrl_p->perf_lock_cluster0_state = 2;
			for_each_possible_cpu(cpu) {
				if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER0_ENABLE_CORE_NUM){
					if (cpu < SHTPS_PERF_LOCK_CLUSTER1_TOP_CPU_ID) {
						#if defined(SHTPS_DEF_CTRL_CPU_CLOCK_LINKED_LCD_BRIGHT_ENABLE)
							if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
								per_cpu(perflock_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER0_CLOCK_FREQUENCY_HIGH_REPORT;
								SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> set [cpu=%d, freq=%u]\n",
														cpu, SHTPS_PERF_LOCK_CLUSTER0_CLOCK_FREQUENCY_HIGH_REPORT);
							}else{
						...
						per_cpu(baselimit_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER0_CLOCK_UPPER_LIMIT_HIGH_REPORT;
						SHTPS_PERF_LOCK_PRINT("baselimit <cluster0> set [cpu=%d, freq=%u]\n",
													cpu, SHTPS_PERF_LOCK_CLUSTER0_CLOCK_UPPER_LIMIT_HIGH_REPORT);
			...
			SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> set sched_set_boost(1)\n");
			sched_set_boost(1);

			SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> high report mode start\n");
		}
		else{
			...
			for_each_possible_cpu(cpu) {
				if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER0_ENABLE_CORE_NUM){
					if (cpu < SHTPS_PERF_LOCK_CLUSTER1_TOP_CPU_ID) {
						...
						per_cpu(baselimit_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER0_CLOCK_UPPER_LIMIT;
						SHTPS_PERF_LOCK_PRINT("baselimit <cluster0> set [cpu=%d, freq=%u]\n",
													cpu, SHTPS_PERF_LOCK_CLUSTER0_CLOCK_UPPER_LIMIT);
...
static void shtps_perflock_cluster0_disable(struct shtps_rmi_spi *ts)
{
	...
	if(ts->cpu_clock_ctrl_p->perf_lock_cluster0_state != 0){
		if(ts->cpu_clock_ctrl_p->perf_lock_cluster0_state == 2){
			SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> set sched_set_boost(0)\n");
			sched_set_boost(0);
		}
...
static void shtps_perflock_cluster1_enable(struct shtps_rmi_spi *ts)
{
	...
	if(req == 1){
		if(ts->fw_report_rate_cur == SHTPS_CTRL_FW_REPORT_RATE_PARAM_HIGH){
			ts->cpu_clock_ctrl_p->perf_lock_cluster1_state = 2;
			for_each_possible_cpu(cpu) {
				if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER1_ENABLE_CORE_NUM){
					if (cpu >= SHTPS_PERF_LOCK_CLUSTER1_TOP_CPU_ID) {
						#if defined(SHTPS_DEF_CTRL_CPU_CLOCK_LINKED_LCD_BRIGHT_ENABLE)
							if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
								per_cpu(perflock_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER1_CLOCK_FREQUENCY_HIGH_REPORT;
								SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> set [cpu=%d, freq=%u]\n",
														cpu, SHTPS_PERF_LOCK_CLUSTER1_CLOCK_FREQUENCY_HIGH_REPORT);
							}else{
								per_cpu(perflock_freq, cpu) = SHTPS_PERF_LOCK_CLOCK_FREQUENCY_LCD_LPM;
								SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> set [cpu=%d, freq=%u]\n",
														cpu, SHTPS_PERF_LOCK_CLOCK_FREQUENCY_LCD_LPM);
						...
						per_cpu(baselimit_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER1_CLOCK_UPPER_LIMIT_HIGH_REPORT;
						SHTPS_PERF_LOCK_PRINT("baselimit <cluster1> set [cpu=%d, freq=%u]\n",
													cpu, SHTPS_PERF_LOCK_CLUSTER1_CLOCK_UPPER_LIMIT_HIGH_REPORT);
			...
			SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> set sched_set_boost(1)\n");
			sched_set_boost(1);

			SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> high report mode start\n");
		}
		else{
			...
			for_each_possible_cpu(cpu) {
				if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER1_ENABLE_CORE_NUM){
					if (cpu >= SHTPS_PERF_LOCK_CLUSTER1_TOP_CPU_ID) {
						...
						per_cpu(baselimit_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER1_CLOCK_UPPER_LIMIT;
						SHTPS_PERF_LOCK_PRINT("baselimit <cluster1> set [cpu=%d, freq=%u]\n",
													cpu, SHTPS_PERF_LOCK_CLUSTER1_CLOCK_UPPER_LIMIT);
...
static void shtps_perflock_cluster1_disable(struct shtps_rmi_spi *ts)
{
	...
	if(ts->cpu_clock_ctrl_p->perf_lock_cluster1_state != 0){
		if(ts->cpu_clock_ctrl_p->perf_lock_cluster1_state == 2){
			SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> set sched_set_boost(0)\n");
			sched_set_boost(0);
		}
...
static int shtps_perflock_ctrl_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	case SH_CPUFREQ_BASE_LIMIT:
		sh_cpufreq_verify_within_limits_max(policy, per_cpu(baselimit_freq, policy->cpu));
		break;
	case CPUFREQ_ADJUST:
...
void shtps_func_check_crc_error(struct shtps_rmi_spi *ts)
{
...
#else
	shtps_fwctl_check_crc_error(ts, &buf);
	...
	if(shtps_fwupdate_builtin_enable(ts) == 0){
		SHTPS_LOG_ERR_PRINT("This touch panel is not a panel to be updated. skip FW Update.\n");
		update = 0;
	}
...
static void shtps_func_cover_on(struct shtps_rmi_spi *ts)
{
	...
	shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_COVER_MODE, 1);
}

static void shtps_func_cover_off(struct shtps_rmi_spi *ts)
{
	shtps_cover_set_state(ts, 0);
	shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_COVER_MODE, 0);
}
...
int shtps_get_lcd_resolution_kind(struct shtps_rmi_spi *ts)
{
	#ifdef CONFIG_OF
		int val;

		if(IS_ERR_OR_NULL(ts->lcd_det_state_active) == 0 && IS_ERR_OR_NULL(ts->lcd_det_state_suspend) == 0){

			gpio_request(ts->lcd_det_gpio, "shtps_lcd_det");
			pinctrl_select_state(ts->pinctrl, ts->lcd_det_state_active);
			mb();
			udelay(10);

			val = gpio_get_value(ts->lcd_det_gpio);
			SHTPS_LOG_DBG_PRINT("%s() val = %d(%s)\n", __func__, val, (val == 1 ? "FHD" : "WQHD"));

			pinctrl_select_state(ts->pinctrl, ts->lcd_det_state_suspend);
			gpio_free(ts->lcd_det_gpio);

			if(val == 1){
				return SHTPS_LCD_RESOLUTION_KIND_FHD;
			}
		}
	#endif /* CONFIG_OF */

	return SHTPS_LCD_RESOLUTION_KIND_WQHD;
}
...
static ssize_t shtps_record_log_kernel_write(struct file *fp, const char *buf, size_t size)
{
	...
	res = vfs_write(fp, buf, size, &fp->f_pos);
...
void shtps_func_request_async( struct shtps_rmi_spi *ts, int event)
{
	...
	if ( msg_p == NULL ){
		SHTPS_LOG_ERR_PRINT("Out of memory [event=%d:%s]\n", event, shtps_func_get_event_name(event));
		return;
	}
...
static void shtps_overlap_event_check(struct shtps_rmi_spi *ts)
{
	...
	list_for_each(list_p, &(ts->queue)){
		...
		if( (ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_LCD_ON) ||
			(ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_LCD_OFF) )
		{
			overlap_req_cnt_lcd++;
		}

		if( (ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_GRIP_ON) ||
			(ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_GRIP_OFF) )
		{
			overlap_req_cnt_grip++;
		}

		if( (ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_COVER_ON) ||
			(ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_COVER_OFF) )
		{
			...
			if(ts->cur_msg_p != NULL){
					...
					(ts->cur_msg_p->event == SHTPS_FUNC_REQ_EVENT_LCD_OFF) )
				{
					if(overlap_req_cnt_lcd > 0){
						...
						overlap_req_cnt_lcd--;
						SHTPS_LOG_DBG_PRINT("FuncReq[%d:%s] overlap canceled\n", ts->cur_msg_p->event, shtps_func_get_event_name(ts->cur_msg_p->event));
					}
				}
...
static void shtps_func_workq( struct work_struct *work_p )
{
	...
	while( list_empty( &(ts->queue) ) == 0 ){
		...
		SHTPS_LOG_DBG_PRINT("FuncReq[%d:%s] start\n", ts->cur_msg_p->event, shtps_func_get_event_name(ts->cur_msg_p->event));

		shtps_func_wakelock(ts, 1);

		switch(ts->cur_msg_p->event){
			case SHTPS_FUNC_REQ_EVENT_OPEN:
			...
			case SHTPS_FUNC_REQ_EVENT_CLOSE:
				#if defined(SHTPS_GUARANTEE_SPI_ACCESS_IN_WAKE_ENABLE)
			...
			case SHTPS_FUNC_REQ_EVENT_ENABLE:
				#if defined(SHTPS_GUARANTEE_SPI_ACCESS_IN_WAKE_ENABLE)
			...
			case SHTPS_FUNC_REQ_EVENT_DISABLE:
				shtps_func_disable(ts);
				ts->cur_msg_p->status = 0;
				break;

			#if defined( SHTPS_CHECK_CRC_ERROR_ENABLE )
				case SHTPS_FUNC_REQ_EVENT_CHECK_CRC_ERROR:
					shtps_func_check_crc_error(ts);
					ts->cur_msg_p->status = 0;
					break;
			#endif /* #if defined( SHTPS_CHECK_CRC_ERROR_ENABLE ) */

			case SHTPS_FUNC_REQ_EVENT_LCD_ON:
				#if defined( SHTPS_DEF_RECORD_LOG_FILE_ENABLE )
			...
			case SHTPS_FUNC_REQ_EVENT_LCD_OFF:
				#if defined( SHTPS_DEF_RECORD_LOG_FILE_ENABLE )
			...
			#if defined(SHTPS_DEF_LPWG_GRIP_PROC_ASYNC_ENABLE)
				case SHTPS_FUNC_REQ_EVENT_GRIP_ON:
					shtps_func_grip_on(ts);
					ts->cur_msg_p->status = 0;
					break;

				case SHTPS_FUNC_REQ_EVENT_GRIP_OFF:
			...
			#if defined(SHTPS_PROXIMITY_SUPPORT_ENABLE)
				case SHTPS_FUNC_REQ_EVENT_PROXIMITY_CHECK:
			...
			#if defined(SHTPS_COVER_ENABLE)
				case SHTPS_FUNC_REQ_EVENT_COVER_ON:
					shtps_func_cover_on(ts);
					ts->cur_msg_p->status = 0;
					break;

				case SHTPS_FUNC_REQ_EVENT_COVER_OFF:
		...
		SHTPS_LOG_DBG_PRINT("FuncReq[%d:%s] end\n", ts->cur_msg_p->event, shtps_func_get_event_name(ts->cur_msg_p->event));
...
{
	switch(event){
		case SHTPS_FUNC_REQ_EVENT_OPEN:				return "OPEN";
		case SHTPS_FUNC_REQ_EVENT_CLOSE:			return "CLOSE";
		case SHTPS_FUNC_REQ_EVENT_ENABLE:			return "ENABLE";
		case SHTPS_FUNC_REQ_EVENT_DISABLE:			return "DISABLE";
		case SHTPS_FUNC_REQ_EVENT_CHECK_CRC_ERROR:	return "CHECK_CRC_ERROR";
		case SHTPS_FUNC_REQ_EVENT_PROXIMITY_CHECK:	return "PROXIMITY_CHECK";
		case SHTPS_FUNC_REQ_EVENT_LCD_ON:			return "LCD_ON";
		case SHTPS_FUNC_REQ_EVENT_LCD_OFF:			return "LCD_OFF";
		case SHTPS_FUNC_REQ_EVENT_GRIP_ON:			return "GRIP_ON";
		case SHTPS_FUNC_REQ_EVENT_GRIP_OFF:			return "GRIP_OFF";
		case SHTPS_FUNC_REQ_EVENT_COVER_ON:			return "COVER_ON";
		case SHTPS_FUNC_REQ_EVENT_COVER_OFF:		return "COVER_OFF";
		case SHTPS_FUNC_REQ_BOOT_FW_UPDATE:			return "BOOT_FW_UPDATE";
	}
	return "UnKnown";
}
